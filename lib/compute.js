...
function Compute(schema, options) {
  ...
  self.coll1 = mongolib.connect(self.options.connectionURI).collection(self.options.collectionName);
  self.coll2 = mongolib.connect(self.options.connectionURI).collection(self.options.collectionName + '_corpus');
  self.schema = schema;
  self.bank = {};
