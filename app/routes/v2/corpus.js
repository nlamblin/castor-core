...
module.exports = function(core) {
  ...
  .append('recordsTotal', function(req, fill) {
      ...
      core.connect().then(function(db) {
          db.collection(self.mongoCollection + '_corpus').find(this.selector).count().then(fill).catch(fill);
      }).catch(fill);
  })
  ...
  .complete('recordsFiltered', function(req, fill) {
      ...
      core.connect().then(function(db) {
          db.collection(self.mongoCollection + '_corpus').find(this.mongoQuery, this.mongoOptions).count().then(fill).catch(fill);
      }).catch(fill);
  })
  .complete('data', function(req, fill) {
      ...
      core.connect().then(function(db) {
          db.collection(self.mongoCollection + '_corpus').find(self.mongoQuery, self.mongoOptions).sort(self.mongoSort).skip(self.parameters.startIndex).limit(self.parameters.itemsPerPage).toArray().then(func).catch(fill);
      }).catch(fill);
  })
