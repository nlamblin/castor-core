...
module.exports = function(model) {
  ...
  .declare('indexDescription', function(req, fill) {
      var index = {
        ...
        "_hash": null,
        "_template": "<ul><li>_wid : {{_wid}}</li><li>_url : <a href=\"{{_url|e}}\">{{$_url}}</a></li></ul><ol> {% for doc in _documents %} <li><ul><li>_wid : {{_wid}}</li><li>_label : {{_label}}</li><li>_text : {{_text}}</li><li>_hash : {{_hash}}</li></ul></li> {% endfor %}</ol>",
        "_root": true,
        "_columns" : {
          "_wid" : {
            //
